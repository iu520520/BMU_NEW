...
static void cell_balance_delay_ms(uint32_t delay_ms)
{
    if (xTaskGetSchedulerState() == taskSCHEDULER_RUNNING)
    {
        /*
         * 多等待一个节拍，保证实际等待时间不会因为刚好跨越Tick边界
         * 而短于变压器释放能量或模拟开关稳定所需的时间。
         */
        vTaskDelay(pdMS_TO_TICKS(delay_ms) + 1u);
    }
    else
    {
...
static void cell_balance_hw_stop(bool wait_for_energy_release)
{
    ...
    {
        cell_balance_delay_ms(CELL_BALANCE_SWITCH_DEADTIME_MS);
    }
...
static void cell_balance_hw_start(uint8_t cell_number)//cell_number为开启的电池编号，1-16
{
    ...
    cell_balance_delay_ms(CELL_BALANCE_PATH_SETTLE_MS);
