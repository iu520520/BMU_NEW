...
static volatile size_t s_rx_head;//中断写入位置，指向下一个可写入的下标
static volatile size_t s_rx_tail;//主循环读取位置，指向下一个待读取的下标
...
/* 进入短临界区并返回进入前的全局中断状态。 */
static uint32_t pc_uart_enter_critical(void)
{
...
/* 仅当进入临界区前允许中断时，才重新打开全局中断。 */
static void pc_uart_exit_critical(uint32_t primask)
{
    if (primask == 0u)
    {
        __enable_irq();
...
{ 
    uint32_t primask = pc_uart_enter_critical();
    ...
    pc_uart_exit_critical(primask);
} 

/* UART4接收中断入口：读取DAT会清除RXDNE，避免中断被连续重复触发。 */
void pc_uart_irq_handler(void)
{
    uint16_t flags = (uint16_t)BOARD_PC_UART->STS;
    uint16_t errors = (uint16_t)(flags & PC_UART_ERROR_FLAGS);

    if ((flags & (USART_FLAG_RXDNE | PC_UART_ERROR_FLAGS)) == 0u)
    {
        return;
    }

    /*
     * 必须先读取STS、再读取DAT：正常接收时清除RXDNE，
     * 发生ORE/FE/NE/PE时同时完成硬件要求的错误标志清除序列。
     */
    uint8_t data = (uint8_t)USART_ReceiveData(BOARD_PC_UART);
    ...
    {
        pc_uart_store_received_byte(data);
    }
}
...
{ 
    ...
    return PC_UART_RX_BUFFER_SIZE - tail + head;
} 
